...
describe('Widgets/WTooltip', () => {
    ...
    xit(`Should be render correctly with label`, () => {
        const wrapper = shallowMount(WTooltip, defaultConfig)
        wrapper.setData({ selected: { label: 'test' } })
        expect(wrapper.html()).toMatchSnapshot()
    })

    xit(`Should be render correctly with values`, () => {
        const wrapper = shallowMount(WTooltip, defaultConfig)
    ...
    describe('Watchers', () => {
        xit(`Shouldn't be set selected if changed Chart.active`, (done) => {
        ...
        xit(`Shouldn't be reset selected if changed Chart.active`, (done) => {
            const wrapper = mount(WTooltip, defaultConfig)
            wrapper.vm.Chart = {
                ...provide.Chart,
    ...
    describe('Methods', () => {
        xit(`Should be set visible prop to true after call show method`, () => {
            const wrapper = shallowMount(WTooltip, defaultConfig)
            wrapper.vm.show()
            expect(wrapper.vm.visible).toBeTruthy()
        })

        xit(`Should be set visible prop to false after call hide method`, () => {
            const wrapper = shallowMount(WTooltip, defaultConfig)
            wrapper.vm.hide()
            expect(wrapper.vm.visible).toBeFalsy()
        })

        xit(`Should be set visible prop to false and select prop after call reset method`, () => {
            const wrapper = shallowMount(WTooltip, defaultConfig)
            wrapper.vm.reset()
            expect(wrapper.vm.visible).toBeFalsy()
        ...
        xit(`Should be set x and y prop after call calcPos method`, () => {
            const wrapper = shallowMount(WTooltip, defaultConfig)
            wrapper.setData({ selected })
